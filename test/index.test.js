...
describe('实例方法', () => {
  ...
  describe('isBetween', () => {
    ...
    test('年', () => {
      ...
      expect(eiya.isBetween(new Eiya(2019, 0, 1), new Eiya(2020, 3, 4), {precision: 'year', left: 'open', right: 'close'})).toBeTruthy();
      expect(eiya.isBetween(new Eiya(2020, 0, 1), new Eiya(2020, 3, 4), {precision: 'year', left: 'close', right: 'close'})).toBeTruthy();
    })
    test('月', () => {
      expect(eiya.isBetween(new Eiya(2019, 6, 1), new Eiya(2021, 3, 4), 'month')).toBeTruthy()
      expect(eiya.isBetween(new Eiya(2019, 6, 1), new Eiya(2021, 3, 4), {precision: 'month', easy: true})).toBeFalsy()
    })
    test('日', () => {
      expect(eiya.isBetween(new Eiya(2019, 4, 1), new Eiya(2021, 3, 4), 'date')).toBeTruthy()
      expect(eiya.isBetween(new Eiya(2020, 3, 4), new Eiya(2021, 3, 4), {precision: 'date', left: 'open'})).toBeFalsy()
    })
    test('时', () => {
      expect(eiya.isBetween(new Eiya(2019, 3, 4, 14), new Eiya(2020, 3, 4, 13), 'hour')).toBeFalsy()
    })
    test('分', () => {
      expect(eiya.isBetween(new Eiya(2020, 3, 4, 14, 15), new Eiya(2020, 3, 4, 14, 15), {precision: 'minute', right: 'open'})).toBeFalsy()
    })
  })
...
describe('异常', () => {
  ...
  test('H不能和h同时存在', () => {
    expect(() => Eiya.parse('2020/10/04 09:34:55', 'yyyy/MM/dd HH:hh:ss')).toThrow('非法格式字符串: H不能和h,a同时存在');
  })
  test('H不能和a同时存在', () => {
    expect(() => Eiya.parse('2020/10/04 09:34:55 pm', 'yyyy/MM/dd HH:mm:ss a')).toThrow('非法格式字符串: H不能和h,a同时存在');
  })
  test('h和a必须成对出现', () => {
